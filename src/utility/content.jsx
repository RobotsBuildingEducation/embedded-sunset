...
- \`data_set && data_object\` are also fundamentally equivalent.
  ...
  Stay focused and best of luck with the rest!
    ...
    question: {
      ...
      previewCode: "function TypeInspectorPreview() {\n  const [selected, setSelected] = React.useState('42');\n  const types = [\n    { label: '42', type: 'number', color: '#3b82f6', desc: 'Numeric value used for math and calculations' },\n    { label: \"'42'\", type: 'string', color: '#f59e0b', desc: 'Text characters wrapped in quotes' },\n    { label: 'true', type: 'boolean', color: '#8b5cf6', desc: 'Logical true / false boolean flag' },\n    { label: 'null', type: 'object', color: '#64748b', desc: 'Intentional absence of any object value' }\n  ];\n  const cur = types.find(t => t.label === selected) || types[0];\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>typeof {selected}:</span>\n        <span style={{ background: cur.color, color: 'white', fontWeight: 800, padding: '2px 8px', borderRadius: 6, fontSize: 12 }}>\"{cur.type}\"</span>\n      </div>\n      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(68px, 1fr))', gap: 6, marginBottom: 8 }}>\n        {types.map(t => (\n          <button key={t.label} onClick={() => setSelected(t.label)} style={{ minHeight: 34, padding: '4px 8px', borderRadius: 6, border: '1.5px solid', borderColor: selected === t.label ? '#3b82f6' : '#cbd5e1', background: selected === t.label ? '#eff6ff' : 'white', color: selected === t.label ? '#1d4ed8' : '#334155', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>{t.label}</button>\n        ))}\n      </div>\n      <div style={{ fontSize: 11, color: '#64748b', background: 'white', padding: '6px 8px', borderRadius: 6, border: '1px solid #e2e8f0' }}>\n        {cur.desc}\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function VariableKeywordsPreview() {\n  const [selected, setSelected] = React.useState('let');\n  const items = [\n    { kw: 'let', valid: true, sample: 'let x = 10;', desc: '\u2713 Valid: Declares a re-assignable variable' },\n    { kw: 'const', valid: true, sample: 'const y = 20;', desc: '\u2713 Valid: Declares a block-scoped constant' },\n    { kw: 'style', valid: false, sample: 'style z = 30;', desc: '\u2717 Invalid: \"style\" is a CSS/property word, not a declaration keyword' },\n    { kw: 'return', valid: false, sample: 'return w = 40;', desc: '\u2717 Invalid: \"return\" exits functions, does not declare variables' }\n  ];\n  const cur = items.find(i => i.kw === selected) || items[0];\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>Variable Keyword Tester</span>\n        <span style={{ background: cur.valid ? '#10b981' : '#ef4444', color: 'white', fontWeight: 800, padding: '2px 8px', borderRadius: 6, fontSize: 12 }}>{cur.valid ? '\u2713 Valid Keyword' : '\u2717 Invalid'}</span>\n      </div>\n      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(68px, 1fr))', gap: 6, marginBottom: 8 }}>\n        {items.map(it => (\n          <button key={it.kw} onClick={() => setSelected(it.kw)} style={{ minHeight: 34, padding: '4px 8px', borderRadius: 6, border: '1.5px solid', borderColor: selected === it.kw ? (it.valid ? '#10b981' : '#ef4444') : '#cbd5e1', background: selected === it.kw ? (it.valid ? '#dcfce7' : '#fee2e2') : 'white', color: selected === it.kw ? (it.valid ? '#15803d' : '#b91c1c') : '#334155', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>{it.kw}</button>\n        ))}\n      </div>\n      <div style={{ padding: '6px 8px', background: 'white', borderRadius: 6, border: '1px solid #e2e8f0', fontFamily: 'monospace', fontSize: 11.5, color: '#0f172a', marginBottom: 4 }}>\n        {cur.sample}\n      </div>\n      <div style={{ fontSize: 10.5, color: cur.valid ? '#15803d' : '#b91c1c', fontWeight: 600 }}>\n        {cur.desc}\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function VariableAccumulatorPreview() {\n  const [step, setStep] = React.useState(0);\n  const steps = [\n    { label: 'Step 1: let count = 1;', val: 1, desc: 'Variable count created with initial value 1' },\n    { label: 'Step 2: count += 2;', val: 3, desc: 'Adds 2 to previous count (1 + 2 = 3)' },\n    { label: 'Step 3: console.log(count);', val: 3, desc: 'Prints final accumulated value 3 to console' }\n  ];\n  const cur = steps[step];\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 12, fontWeight: 700, color: '#334155' }}>Live Variable State</span>\n        <span style={{ background: '#3b82f6', color: 'white', fontWeight: 800, padding: '3px 8px', borderRadius: 6, fontSize: 13 }}>count = {cur.val}</span>\n      </div>\n      <div style={{ padding: '8px 10px', background: 'white', borderRadius: 6, border: '1px solid #e2e8f0', marginBottom: 8 }}>\n        <div style={{ fontSize: 12, fontWeight: 700, color: '#0f172a', fontFamily: 'monospace' }}>{cur.label}</div>\n        <div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>{cur.desc}</div>\n      </div>\n      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>\n        <button onClick={() => setStep(s => Math.min(steps.length - 1, s + 1))} disabled={step === steps.length - 1} style={{ flex: '1 1 120px', minHeight: 34, padding: '4px 8px', borderRadius: 6, background: step === steps.length - 1 ? '#cbd5e1' : '#ec4899', color: 'white', border: 0, fontWeight: 700, fontSize: 11.5, cursor: step === steps.length - 1 ? 'default' : 'pointer' }}>Next Line \u2192</button>\n        <button onClick={() => setStep(0)} style={{ minHeight: 34, padding: '4px 10px', borderRadius: 6, background: '#f1f5f9', color: '#475569', border: '1px solid #cbd5e1', fontWeight: 600, fontSize: 11.5, cursor: 'pointer' }}>Reset</button>\n      </div>\n    </div>\n  );\n}",
      code: "let count = 1;\ncount += 2;\nconsole.log(count);",
    ...
    question: {
      ...
      previewCode: "function ConstLetMutationPreview() {\n  const [useLet, setUseLet] = React.useState(false);\n  const [score, setScore] = React.useState(1);\n  const [error, setError] = React.useState(false);\n  const handleIncrement = () => {\n    if (!useLet) {\n      setError(true);\n    } else {\n      setScore(s => s + 1);\n      setError(false);\n    }\n  };\n  const handleToggle = (toLet) => {\n    setUseLet(toLet);\n    setError(false);\n    setScore(1);\n  };\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>Declaration Keyword:</span>\n        <div style={{ display: 'flex', gap: 4 }}>\n          <button onClick={() => handleToggle(false)} style={{ padding: '3px 8px', borderRadius: 5, border: '1px solid', borderColor: !useLet ? '#ef4444' : '#cbd5e1', background: !useLet ? '#fee2e2' : 'white', color: !useLet ? '#b91c1c' : '#64748b', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>const (Bug)</button>\n          <button onClick={() => handleToggle(true)} style={{ padding: '3px 8px', borderRadius: 5, border: '1px solid', borderColor: useLet ? '#10b981' : '#cbd5e1', background: useLet ? '#dcfce7' : 'white', color: useLet ? '#15803d' : '#64748b', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>let (Fix \u2713)</button>\n        </div>\n      </div>\n      <div style={{ padding: '8px 10px', background: 'white', borderRadius: 6, border: '1px solid #e2e8f0', marginBottom: 8, fontFamily: 'monospace', fontSize: 11.5 }}>\n        <div>{useLet ? 'let' : 'const'} score = 1;</div>\n        <div style={{ color: '#64748b' }}>score += 1;</div>\n      </div>\n      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 6 }}>\n        <button onClick={handleIncrement} style={{ minHeight: 34, padding: '4px 12px', borderRadius: 6, background: '#3b82f6', color: 'white', border: 0, fontWeight: 700, fontSize: 11.5, cursor: 'pointer' }}>Run score += 1</button>\n        {error ? (\n          <span style={{ background: '#fee2e2', color: '#b91c1c', padding: '3px 8px', borderRadius: 6, fontSize: 11, fontWeight: 700 }}>\u2717 TypeError: Assignment to constant</span>\n        ) : (\n          <span style={{ background: '#dcfce7', color: '#15803d', padding: '3px 8px', borderRadius: 6, fontSize: 11.5, fontWeight: 800 }}>\u2713 score = {score}</span>\n        )}\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function ArrowFunctionPreview() {\n  const [a, setA] = React.useState(3);\n  const [b, setB] = React.useState(5);\n  const result = a + b;\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>Arrow Function: const add = (a, b) =&gt; a + b;</span>\n        <span style={{ background: '#3b82f6', color: 'white', fontWeight: 800, padding: '2px 8px', borderRadius: 6, fontSize: 12.5 }}>add({a}, {b}) = {result}</span>\n      </div>\n      <div style={{ display: 'flex', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>\n        <div style={{ flex: '1 1 100px' }}>\n          <label style={{ fontSize: 10.5, color: '#64748b', fontWeight: 600, display: 'block', marginBottom: 2 }}>Param a: {a}</label>\n          <input type=\"range\" min=\"1\" max=\"20\" value={a} onChange={(e) => setA(Number(e.target.value))} style={{ width: '100%' }} />\n        </div>\n        <div style={{ flex: '1 1 100px' }}>\n          <label style={{ fontSize: 10.5, color: '#64748b', fontWeight: 600, display: 'block', marginBottom: 2 }}>Param b: {b}</label>\n          <input type=\"range\" min=\"1\" max=\"20\" value={b} onChange={(e) => setB(Number(e.target.value))} style={{ width: '100%' }} />\n        </div>\n      </div>\n      <div style={{ padding: '6px 8px', background: '#0f172a', borderRadius: 6, color: '#38bdf8', fontSize: 11, fontFamily: 'monospace', overflowX: 'auto', whiteSpace: 'nowrap' }}>\n        <span style={{ color: '#f472b6' }}>const</span> add = ({a}, {b}) =&gt; {a} + {b} <span style={{ color: '#10b981' }}>// returns {result}</span>\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function GradeBranchPreview() {\n  const [score, setScore] = React.useState(85);\n  const grade = score >= 90 ? 'A' : score >= 80 ? 'B' : 'C';\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 12, fontWeight: 700, color: '#334155' }}>Score: {score}</span>\n        <span style={{ background: grade === 'A' ? '#10b981' : grade === 'B' ? '#3b82f6' : '#f59e0b', color: 'white', fontWeight: 800, padding: '2px 8px', borderRadius: 6, fontSize: 13 }}>Grade: {grade}</span>\n      </div>\n      <input type=\"range\" min=\"50\" max=\"100\" value={score} onChange={(e) => setScore(Number(e.target.value))} style={{ width: '100%', marginBottom: 10 }} />\n      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 11, fontFamily: 'monospace' }}>\n        <div style={{ padding: '4px 6px', borderRadius: 4, background: score >= 90 ? '#dcfce7' : '#f1f5f9', color: score >= 90 ? '#15803d' : '#94a3b8', borderLeft: score >= 90 ? '3px solid #22c55e' : '3px solid transparent', overflowX: 'auto', whiteSpace: 'nowrap' }}>if (score &gt;= 90) return \"A\" {score >= 90 ? '\u2713 Active' : ''}</div>\n        <div style={{ padding: '4px 6px', borderRadius: 4, background: score < 90 && score >= 80 ? '#dbeafe' : '#f1f5f9', color: score < 90 && score >= 80 ? '#1d4ed8' : '#94a3b8', borderLeft: score < 90 && score >= 80 ? '3px solid #3b82f6' : '3px solid transparent', overflowX: 'auto', whiteSpace: 'nowrap' }}>else if (score &gt;= 80) return \"B\" {score < 90 && score >= 80 ? '\u2713 Active' : ''}</div>\n        <div style={{ padding: '4px 6px', borderRadius: 4, background: score < 80 ? '#fef3c7' : '#f1f5f9', color: score < 80 ? '#b45309' : '#94a3b8', borderLeft: score < 80 ? '3px solid #f59e0b' : '3px solid transparent', overflowX: 'auto', whiteSpace: 'nowrap' }}>else return \"C\" {score < 80 ? '\u2713 Active' : ''}</div>\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function AgeBoundaryPreview() {\n  const [age, setAge] = React.useState(18);\n  const bugResult = age > 18;\n  const fixResult = age >= 18;\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 12, fontWeight: 700, color: '#334155' }}>Age Input: {age}</span>\n        <span style={{ fontSize: 11, color: '#64748b' }}>Adult cutoff: 18+</span>\n      </div>\n      <input type=\"range\" min=\"15\" max=\"21\" value={age} onChange={(e) => setAge(Number(e.target.value))} style={{ width: '100%', marginBottom: 10 }} />\n      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>\n        <div style={{ flex: '1 1 120px', padding: '6px 8px', background: bugResult ? '#dcfce7' : '#fee2e2', borderRadius: 6, border: '1px solid', borderColor: bugResult ? '#86efac' : '#fca5a5' }}>\n          <div style={{ fontSize: 10, color: '#64748b', fontWeight: 700 }}>Bug: if (age &gt; 18)</div>\n          <div style={{ fontSize: 12, fontWeight: 800, color: bugResult ? '#15803d' : '#b91c1c' }}>{bugResult ? '\u2713 Allowed' : '\u2717 Denied'}</div>\n        </div>\n        <div style={{ flex: '1 1 120px', padding: '6px 8px', background: fixResult ? '#dcfce7' : '#fee2e2', borderRadius: 6, border: '1px solid', borderColor: fixResult ? '#86efac' : '#fca5a5' }}>\n          <div style={{ fontSize: 10, color: '#64748b', fontWeight: 700 }}>Fixed: if (age &gt;= 18)</div>\n          <div style={{ fontSize: 12, fontWeight: 800, color: fixResult ? '#15803d' : '#b91c1c' }}>{fixResult ? '\u2713 Allowed' : '\u2717 Denied'}</div>\n        </div>\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function TruthyFalsyPreview() {\n  const [selected, setSelected] = React.useState('0');\n  const values = [\n    { label: '0', val: 0 },\n    { label: '\"\"', val: '' },\n    { label: 'null', val: null },\n    { label: 'undefined', val: undefined },\n    { label: 'NaN', val: NaN },\n    { label: 'false', val: false },\n    { label: '\"0\"', val: '0' },\n    { label: '[]', val: [] },\n    { label: '{}', val: {} },\n  ];\n  const activeItem = values.find(v => v.label === selected);\n  const isTruthy = Boolean(activeItem ? activeItem.val : false);\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 10, border: '1px solid #cbd5e1' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 13, fontWeight: 700, color: '#334155' }}>Boolean({selected}) evaluates to:</span>\n        <span style={{ background: isTruthy ? '#10b981' : '#ef4444', color: 'white', fontWeight: 800, padding: '4px 10px', borderRadius: 6, fontSize: 13 }}>{isTruthy ? '\u2713 Truthy' : '\u2717 Falsy'}</span>\n      </div>\n      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(64px, 1fr))', gap: 6 }}>\n        {values.map(v => (\n          <button key={v.label} onClick={() => setSelected(v.label)} style={{ minHeight: 38, padding: '6px 8px', borderRadius: 7, border: '1.5px solid', borderColor: selected === v.label ? '#2563eb' : '#cbd5e1', background: selected === v.label ? '#dbeafe' : 'white', color: selected === v.label ? '#1e40af' : '#1e293b', fontWeight: 700, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{v.label}</button>\n        ))}\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function GuardClausePreview() {\n  const [amount, setAmount] = React.useState(50);\n  const isInvalid = amount <= 0;\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 12, fontWeight: 700, color: '#334155' }}>Payment: ${amount}</span>\n        <span style={{ background: isInvalid ? '#ef4444' : '#10b981', color: 'white', fontWeight: 800, padding: '2px 8px', borderRadius: 6, fontSize: 12 }}>{isInvalid ? 'Early Return: Invalid' : 'Approved: $' + amount}</span>\n      </div>\n      <input type=\"range\" min=\"-20\" max=\"100\" value={amount} onChange={(e) => setAmount(Number(e.target.value))} style={{ width: '100%', marginBottom: 10 }} />\n      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 11, fontFamily: 'monospace' }}>\n        <div style={{ padding: '4px 6px', borderRadius: 4, background: isInvalid ? '#fee2e2' : '#f1f5f9', color: isInvalid ? '#b91c1c' : '#94a3b8', borderLeft: isInvalid ? '3px solid #ef4444' : '3px solid transparent', overflowX: 'auto', whiteSpace: 'nowrap' }}>if (amount &lt;= 0) return 'Invalid amount' {isInvalid ? '\u2190 Halts Here' : ''}</div>\n        <div style={{ padding: '4px 6px', borderRadius: 4, background: !isInvalid ? '#dcfce7' : '#f1f5f9', color: !isInvalid ? '#15803d' : '#94a3b8', borderLeft: !isInvalid ? '3px solid #22c55e' : '3px solid transparent', overflowX: 'auto', whiteSpace: 'nowrap' }}>return 'Approved' {!isInvalid ? '\u2190 Processed' : '(Skipped)'}</div>\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function LoopTracePreview() {\n  const [stepIndex, setStepIndex] = React.useState(0);\n  const states = [\n    { label: 'Start', i: null, prev: null, expr: 'Initial state before loop', result: 1, isFinal: false },\n    { label: 'i = 0', i: 0, prev: 1, expr: '1 \xD7 2', result: 2, isFinal: false },\n    { label: 'i = 1', i: 1, prev: 2, expr: '2 \xD7 2', result: 4, isFinal: false },\n    { label: 'i = 2', i: 2, prev: 4, expr: '4 \xD7 2', result: 8, isFinal: true },\n  ];\n  const cur = states[stepIndex];\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 10, border: '1px solid #cbd5e1' }}>\n      <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>\n        {states.map((s, idx) => (\n          <button key={s.label} onClick={() => setStepIndex(idx)} style={{ flex: 1, padding: '6px 4px', borderRadius: 6, border: '1px solid', borderColor: stepIndex === idx ? '#3b82f6' : idx < stepIndex ? '#86efac' : '#cbd5e1', background: stepIndex === idx ? '#eff6ff' : idx < stepIndex ? '#f0fdf4' : 'white', color: stepIndex === idx ? '#1d4ed8' : idx < stepIndex ? '#166534' : '#64748b', fontWeight: 700, fontSize: 11.5, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3 }}>\n            {idx < stepIndex && '\u2713 '}{s.label}\n          </button>\n        ))}\n      </div>\n      <div style={{ padding: '10px 12px', background: 'white', borderRadius: 8, border: '1px solid #e2e8f0', marginBottom: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>\n        <div>\n          <div style={{ fontSize: 11, color: '#64748b', fontWeight: 600, marginBottom: 2 }}>\n            {cur.i === null ? 'Variable setup: let count = 1;' : `Iteration ${cur.label} (count *= 2):`}\n          </div>\n          <div style={{ fontSize: 14, fontWeight: 800, color: '#0f172a', fontFamily: 'monospace' }}>\n            {cur.prev !== null ? `${cur.prev} \xD7 2 = ${cur.result}` : 'count = 1'}\n          </div>\n        </div>\n        <div style={{ textAlign: 'right' }}>\n          <span style={{ fontSize: 10, color: '#64748b', fontWeight: 700, display: 'block', textTransform: 'uppercase' }}>Value</span>\n          <span style={{ background: cur.isFinal ? '#10b981' : '#3b82f6', color: 'white', fontWeight: 800, padding: '3px 9px', borderRadius: 6, fontSize: 13 }}>\n            count = {cur.result}\n          </span>\n        </div>\n      </div>\n      <div style={{ display: 'flex', gap: 6 }}>\n        <button onClick={() => setStepIndex(s => Math.min(states.length - 1, s + 1))} disabled={stepIndex === states.length - 1} style={{ flex: 1, minHeight: 38, padding: '6px 12px', borderRadius: 7, background: stepIndex === states.length - 1 ? '#cbd5e1' : '#3b82f6', color: 'white', border: 0, fontWeight: 700, fontSize: 12.5, cursor: stepIndex === states.length - 1 ? 'default' : 'pointer' }}>\n          {stepIndex === states.length - 1 ? 'Loop Complete (console.log \u2192 8)' : 'Next Iteration \u2192'}\n        </button>\n        <button onClick={() => setStepIndex(0)} style={{ minHeight: 38, padding: '6px 14px', borderRadius: 7, background: '#f1f5f9', color: '#475569', border: '1px solid #cbd5e1', fontWeight: 700, fontSize: 12.5, cursor: 'pointer' }}>\n          Reset\n        </button>\n      </div>\n    </div>\n  );\n}",
      code: "let count = 1;\nfor (let i = 0; i < 3; i++) {\n  count *= 2;\n}\nconsole.log(count);",
      options: ["8", "6", "4", "16"],
      answer: "8"
    }
    ...
    question: {
      ...
      previewCode: "function TemplateLiteralPreview() {\n  const [name, setName] = React.useState('Alex');\n  const [role, setRole] = React.useState('Developer');\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>\n        <input value={name} onChange={(e) => setName(e.target.value)} placeholder=\"Name\" style={{ flex: '1 1 90px', minHeight: 32, padding: '4px 8px', fontSize: 11.5, borderRadius: 5, border: '1px solid #cbd5e1', boxSizing: 'border-box' }} />\n        <select value={role} onChange={(e) => setRole(e.target.value)} style={{ flex: '1 1 110px', minHeight: 32, padding: '4px 6px', fontSize: 11.5, borderRadius: 5, border: '1px solid #cbd5e1', background: 'white' }}>\n          <option value=\"Developer\">Developer</option>\n          <option value=\"Designer\">Designer</option>\n          <option value=\"Manager\">Manager</option>\n        </select>\n      </div>\n      <div style={{ padding: '8px 10px', background: '#0f172a', borderRadius: 6, color: '#f8fafc', fontSize: 11.5, fontFamily: 'monospace', overflowX: 'auto', whiteSpace: 'nowrap' }}>\n        <span style={{ color: '#38bdf8' }}>`User </span>\n        <span style={{ background: '#ec4899', color: 'white', padding: '1px 4px', borderRadius: 3 }}>${'{name}'}</span>\n        <span style={{ color: '#38bdf8' }}> is a </span>\n        <span style={{ background: '#8b5cf6', color: 'white', padding: '1px 4px', borderRadius: 3 }}>${'{role}'}</span>\n        <span style={{ color: '#38bdf8' }}>.`</span>\n      </div>\n      <div style={{ marginTop: 6, fontSize: 11.5, color: '#0f172a', fontWeight: 600 }}>\n        Result: <span style={{ color: '#16a34a' }}>\"User {name} is a {role}.\"</span>\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function ArrayMutationPreview() {\n  const [fruits, setFruits] = React.useState(['apple', 'banana', 'cherry']);\n  const [log, setLog] = React.useState('Initial array with 3 fruits.');\n  const handlePush = () => {\n    if (!fruits.includes('date')) {\n      setFruits([...fruits, 'date']);\n      setLog('fruits.push(\"date\") appended to the end.');\n    }\n  };\n  const handleShift = () => {\n    if (fruits.length > 0) {\n      const removed = fruits[0];\n      setFruits(fruits.slice(1));\n      setLog(`fruits.shift() removed \"${removed}\" from index 0.`);\n    }\n  };\n  const handleReset = () => {\n    setFruits(['apple', 'banana', 'cherry']);\n    setLog('Reset array to initial state.');\n  };\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ fontSize: 11, color: '#64748b', marginBottom: 6, wordBreak: 'break-word' }}>{log}</div>\n      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8, minHeight: 32, alignItems: 'center' }}>\n        {fruits.map((f, idx) => (\n          <span key={f + idx} style={{ padding: '3px 8px', borderRadius: 6, background: '#10b981', color: 'white', fontWeight: 700, fontSize: 11 }}>[{idx}] {f}</span>\n        ))}\n        {fruits.length === 0 && <span style={{ color: '#94a3b8', fontSize: 11 }}>Array is empty []</span>}\n      </div>\n      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>\n        <button onClick={handlePush} style={{ flex: '1 1 80px', minHeight: 34, padding: '4px 6px', borderRadius: 5, background: '#3b82f6', color: 'white', border: 0, fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>.push('date')</button>\n        <button onClick={handleShift} style={{ flex: '1 1 80px', minHeight: 34, padding: '4px 6px', borderRadius: 5, background: '#f59e0b', color: 'white', border: 0, fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>.shift()</button>\n        <button onClick={handleReset} style={{ minHeight: 34, padding: '4px 10px', borderRadius: 5, background: '#f1f5f9', color: '#475569', border: '1px solid #cbd5e1', fontWeight: 600, fontSize: 11, cursor: 'pointer' }}>Reset</button>\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function ObjectDestructuringPreview() {\n  const product = { title: 'Headphones', price: 99 };\n  const [unpacked, setUnpacked] = React.useState(true);\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>Source Object: product</span>\n        <button onClick={() => setUnpacked(u => !u)} style={{ padding: '3px 8px', minHeight: 30, borderRadius: 5, border: '1px solid #3b82f6', background: '#eff6ff', color: '#1d4ed8', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>{unpacked ? 'Show Dot Notation' : 'Show Destructuring'}</button>\n      </div>\n      <div style={{ padding: '8px 10px', background: 'white', borderRadius: 6, border: '1px solid #e2e8f0', marginBottom: 8 }}>\n        <div style={{ fontSize: 11, color: '#64748b', marginBottom: 4 }}>{unpacked ? 'const { title, price } = product;' : 'const title = product.title; const price = product.price;' }</div>\n        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>\n          <span style={{ padding: '3px 8px', borderRadius: 6, background: '#dbeafe', color: '#1d4ed8', fontWeight: 700, fontSize: 11 }}>title = \"{product.title}\"</span>\n          <span style={{ padding: '3px 8px', borderRadius: 6, background: '#dcfce7', color: '#15803d', fontWeight: 700, fontSize: 11 }}>price = ${product.price}</span>\n        </div>\n      </div>\n      <div style={{ fontSize: 11.5, color: '#0f172a', fontWeight: 600 }}>\n        Formatted Output: <strong style={{ color: '#ec4899' }}>{product.title}: ${product.price}</strong>\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function ObjectSpreadPreview() {\n  const [theme, setTheme] = React.useState('dark');\n  const original = { user: 'Maya', theme: 'light', volume: 80 };\n  const updated = { ...original, theme };\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>Immutable Object Spread</span>\n        <button onClick={() => setTheme(t => t === 'dark' ? 'light' : 'dark')} style={{ padding: '3px 8px', minHeight: 30, borderRadius: 5, border: '1px solid #8b5cf6', background: '#f5f3ff', color: '#6d28d9', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>Toggle theme: {theme}</button>\n      </div>\n      <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>\n        <div style={{ flex: '1 1 120px', padding: '6px 8px', background: 'white', borderRadius: 6, border: '1px solid #cbd5e1', boxSizing: 'border-box' }}>\n          <div style={{ fontSize: 10, color: '#64748b', fontWeight: 700 }}>original (Untouched)</div>\n          <div style={{ fontSize: 11, fontFamily: 'monospace', color: '#334155' }}>theme: \"{original.theme}\"</div>\n        </div>\n        <div style={{ flex: '1 1 120px', padding: '6px 8px', background: '#f0fdf4', borderRadius: 6, border: '1px solid #86efac', boxSizing: 'border-box' }}>\n          <div style={{ fontSize: 10, color: '#15803d', fontWeight: 700 }}>updated = {'{ ...original, theme }'}</div>\n          <div style={{ fontSize: 11, fontFamily: 'monospace', color: '#166534', fontWeight: 700 }}>theme: \"{updated.theme}\"</div>\n        </div>\n      </div>\n    </div>\n  );\n}",
    ...
    showPreview: true,
    question: {
      questionText: "What occurs during the execution of const student = new User('Maya', 'maya@dev.io')?",
      previewCode: "function ClassInstancePreview() {\n  const [instances, setInstances] = React.useState([\n    { name: 'Maya', email: 'maya@dev.io' }\n  ]);\n  const [name, setName] = React.useState('');\n  const handleSpawn = () => {\n    if (name.trim()) {\n      setInstances([...instances, { name: name.trim(), email: `${name.toLowerCase().replace(/\\s+/g, '')}@dev.io` }]);\n      setName('');\n    }\n  };\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>\n        <input value={name} onChange={(e) => setName(e.target.value)} placeholder=\"User name (e.g. Sam)\" style={{ flex: '1 1 120px', minHeight: 34, padding: '4px 8px', fontSize: 11.5, borderRadius: 6, border: '1px solid #cbd5e1', boxSizing: 'border-box' }} />\n        <button onClick={handleSpawn} style={{ minHeight: 34, padding: '4px 12px', background: '#ec4899', color: 'white', border: 0, borderRadius: 6, fontWeight: 700, fontSize: 11.5, cursor: 'pointer' }}>new User()</button>\n      </div>\n      <div style={{ fontSize: 11, color: '#64748b', marginBottom: 4, fontWeight: 700 }}>Active Class Instances ({instances.length}):</div>\n      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>\n        {instances.map((inst, i) => (\n          <div key={i} style={{ padding: '4px 8px', background: 'white', border: '1px solid #3b82f6', borderRadius: 6, fontSize: 11, wordBreak: 'break-all' }}>\n            <strong style={{ color: '#1d4ed8' }}>{inst.name}</strong> <span style={{ color: '#64748b', fontSize: 10 }}>({inst.email})</span>\n          </div>\n        ))}\n      </div>\n    </div>\n  );\n}",
      options: ["A new object instance is allocated in memory, its constructor executes with 'this' bound to the new instance, and the instance is returned.", "The User class definition is deleted and replaced by a plain function.", "The code serializes the class into a static JSON string on disk.", "The class executes without allocating internal state."],
      answer: "A new object instance is allocated in memory, its constructor executes with 'this' bound to the new instance, and the instance is returned."
    }
  }, {
    group: "2",
    title: "Class and Method Lifecycle Order",
    description: "Sequence the execution milestones of defining and invoking class instances.",
    isSelectOrder: true,
    question: {
      questionText: "Arrange the phases of defining and using a class instance in chronological execution order:",
      options: ["Define the class template with constructor and methods", "Instantiate the object with the new ClassName(args) syntax", "The constructor method executes and initializes properties on this", "Invoke an instance method (e.g. user.getDetails())"],
      answer: ["Define the class template with constructor and methods", "Instantiate the object with the new ClassName(args) syntax", "The constructor method executes and initializes properties on this", "Invoke an instance method (e.g. user.getDetails())"]
    }
  }, {
    group: "2",
    title: "Locating Detached Method Context Bug",
    description: "Identify the line where this becomes undefined when decoupling a method.",
    isRelevantLine: true,
    showPreview: true,
    question: {
      questionText: "Select the line where detaching the method from the instance causes lost 'this' context:",
      previewCode: "function DetachedContextPreview() {\n  const [balance, setBalance] = React.useState(100);\n  const [error, setError] = React.useState(null);\n  const handleMethodCall = () => {\n    setBalance(b => b + 50);\n    setError(null);\n  };\n  const handleDetachedCall = () => {\n    setError('TypeError: Cannot read properties of undefined (reading balance)');\n  };\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>account = new BankAccount(100)</span>\n        <span style={{ background: '#10b981', color: 'white', fontWeight: 800, padding: '2px 8px', borderRadius: 6, fontSize: 12 }}>balance: ${balance}</span>\n      </div>\n      <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>\n        <button onClick={handleMethodCall} style={{ flex: '1 1 120px', minHeight: 34, padding: '4px 6px', borderRadius: 5, background: '#3b82f6', color: 'white', border: 0, fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>account.deposit(50) (this \u2713)</button>\n        <button onClick={handleDetachedCall} style={{ flex: '1 1 120px', minHeight: 34, padding: '4px 6px', borderRadius: 5, background: '#ef4444', color: 'white', border: 0, fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>detached(50) (this \u2717)</button>\n      </div>\n      {error && (\n        <div style={{ padding: '6px 8px', background: '#fee2e2', borderRadius: 6, color: '#b91c1c', fontSize: 11, fontFamily: 'monospace', overflowX: 'auto', whiteSpace: 'nowrap' }}>\n          \u2717 {error}\n        </div>\n      )}\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function ArrayTransformPreview() {\n  const [users, setUsers] = React.useState([\n    { id: 1, name: 'Alice', isActive: true },\n    { id: 2, name: 'Bob', isActive: false },\n    { id: 3, name: 'Charlie', isActive: true },\n  ]);\n  const activeNames = users.filter(u => u.isActive).map(u => u.name);\n  const toggleUser = (id) => {\n    setUsers(users.map(u => u.id === id ? { ...u, isActive: !u.isActive } : u));\n  };\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ fontSize: 11, fontWeight: 700, color: '#64748b', marginBottom: 6 }}>Toggle User Status:</div>\n      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>\n        {users.map(u => (\n          <button key={u.id} onClick={() => toggleUser(u.id)} style={{ flex: '1 1 80px', minHeight: 34, padding: '5px 8px', borderRadius: 6, border: '1px solid', borderColor: u.isActive ? '#10b981' : '#cbd5e1', background: u.isActive ? '#dcfce7' : 'white', color: u.isActive ? '#15803d' : '#64748b', fontSize: 11.5, fontWeight: 700, cursor: 'pointer', textAlign: 'center' }}>{u.name}: {u.isActive ? 'Active \u2713' : 'Inactive'}</button>\n        ))}\n      </div>\n      <div style={{ padding: '6px 8px', background: '#0f172a', borderRadius: 6, color: '#f8fafc', fontSize: 11, fontFamily: 'monospace', overflowX: 'auto', whiteSpace: 'nowrap' }}>\n        <span style={{ color: '#38bdf8' }}>activeNames</span> = [{activeNames.map(n => `\"${n}\"`).join(', ')}]\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function ObjectReferencePreview() {\n  const [score, setScore] = React.useState(25);\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>Shared Object in Heap Memory</span>\n        <span style={{ background: '#8b5cf6', color: 'white', fontWeight: 800, padding: '2px 8px', borderRadius: 6, fontSize: 12 }}>score = {score}</span>\n      </div>\n      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>\n        <div style={{ flex: '1 1 120px', padding: '6px 8px', background: 'white', borderRadius: 6, border: '1px solid #3b82f6', boxSizing: 'border-box' }}>\n          <div style={{ fontSize: 10, color: '#1d4ed8', fontWeight: 700 }}>userA \u2192 (Pointer)</div>\n          <div style={{ fontSize: 12, fontWeight: 800, color: '#0f172a' }}>score: {score}</div>\n        </div>\n        <div style={{ flex: '1 1 120px', padding: '6px 8px', background: 'white', borderRadius: 6, border: '1px solid #ec4899', boxSizing: 'border-box' }}>\n          <div style={{ fontSize: 10, color: '#be185d', fontWeight: 700 }}>userB = userA \u2192</div>\n          <div style={{ fontSize: 12, fontWeight: 800, color: '#0f172a' }}>score: {score}</div>\n        </div>\n      </div>\n      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>\n        <span style={{ fontSize: 10.5, color: '#64748b', fontWeight: 600 }}>Mutate userB.score:</span>\n        <input type=\"range\" min=\"10\" max=\"50\" value={score} onChange={(e) => setScore(Number(e.target.value))} style={{ flex: '1 1 100px', minWidth: 100 }} />\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function OptionalChainingPreview() {\n  const [hasAddress, setHasAddress] = React.useState(false);\n  const user = hasAddress ? { profile: { address: { city: 'Tokyo' } } } : { profile: null };\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>user.profile state:</span>\n        <button onClick={() => setHasAddress(a => !a)} style={{ padding: '4px 8px', minHeight: 30, borderRadius: 5, border: '1px solid #3b82f6', background: '#eff6ff', color: '#1d4ed8', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>Toggle {hasAddress ? 'Address: Tokyo' : 'profile: null'}</button>\n      </div>\n      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 11, fontFamily: 'monospace' }}>\n        <div style={{ padding: '4px 6px', borderRadius: 4, background: hasAddress ? '#dcfce7' : '#fee2e2', color: hasAddress ? '#15803d' : '#b91c1c', overflowX: 'auto', whiteSpace: 'nowrap' }}>user.profile.address.city \u2192 {hasAddress ? '\"Tokyo\"' : '\u2717 TypeError: Cannot read property' }</div>\n        <div style={{ padding: '4px 6px', borderRadius: 4, background: '#dcfce7', color: '#15803d', overflowX: 'auto', whiteSpace: 'nowrap' }}>user?.profile?.address?.city \u2192 {hasAddress ? '\"Tokyo\"' : 'undefined (\u2713 Safe)'}</div>\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function ClassMethodStatePreview() {\n  const [count, setCount] = React.useState(5);\n  const [history, setHistory] = React.useState(['new Counter(5) \u2192 count = 5']);\n  const handleIncrement = (by) => {\n    setCount(prev => {\n      const next = prev + by;\n      setHistory(h => [...h.slice(-2), `increment(${by}) \u2192 count = ${next}`]);\n      return next;\n    });\n  };\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>c = new Counter(5)</span>\n        <span style={{ background: '#ec4899', color: 'white', fontWeight: 800, padding: '2px 8px', borderRadius: 6, fontSize: 12.5 }}>c.count = {count}</span>\n      </div>\n      <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>\n        <button onClick={() => handleIncrement(3)} style={{ flex: '1 1 100px', minHeight: 34, padding: '4px 6px', borderRadius: 5, background: '#3b82f6', color: 'white', border: 0, fontWeight: 700, fontSize: 11.5, cursor: 'pointer' }}>c.increment(3)</button>\n        <button onClick={() => handleIncrement(1)} style={{ flex: '1 1 100px', minHeight: 34, padding: '4px 6px', borderRadius: 5, background: '#10b981', color: 'white', border: 0, fontWeight: 700, fontSize: 11.5, cursor: 'pointer' }}>c.increment(1)</button>\n      </div>\n      <div style={{ padding: '6px 8px', background: '#0f172a', borderRadius: 6, color: '#38bdf8', fontSize: 11, fontFamily: 'monospace' }}>\n        {history.map((line, idx) => (\n          <div key={idx}>{line}</div>\n        ))}\n      </div>\n    </div>\n  );\n}",
...
Stay focused and good luck with the rest!
    ...
    question: {
      ...
      previewCode: "function TypeInspectorPreview() {\n  const [seleccionado, setSeleccionado] = React.useState('42');\n  const tipos = [\n    { etiqueta: '42', tipo: 'number', color: '#3b82f6', desc: 'Valor num\xE9rico utilizado para operaciones matem\xE1ticas' },\n    { etiqueta: \"'42'\", tipo: 'string', color: '#f59e0b', desc: 'Texto entre comillas' },\n    { etiqueta: 'true', tipo: 'boolean', color: '#8b5cf6', desc: 'Bandera l\xF3gica verdadero / falso' },\n    { etiqueta: 'null', tipo: 'object', color: '#64748b', desc: 'Ausencia intencional de valor' }\n  ];\n  const actual = tipos.find(t => t.etiqueta === seleccionado) || tipos[0];\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>typeof {seleccionado}:</span>\n        <span style={{ background: actual.color, color: 'white', fontWeight: 800, padding: '2px 8px', borderRadius: 6, fontSize: 12 }}>\"{actual.tipo}\"</span>\n      </div>\n      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(68px, 1fr))', gap: 6, marginBottom: 8 }}>\n        {tipos.map(t => (\n          <button key={t.etiqueta} onClick={() => setSeleccionado(t.etiqueta)} style={{ minHeight: 34, padding: '4px 8px', borderRadius: 6, border: '1.5px solid', borderColor: seleccionado === t.etiqueta ? '#3b82f6' : '#cbd5e1', background: seleccionado === t.etiqueta ? '#eff6ff' : 'white', color: seleccionado === t.etiqueta ? '#1d4ed8' : '#334155', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>{t.etiqueta}</button>\n        ))}\n      </div>\n      <div style={{ fontSize: 11, color: '#64748b', background: 'white', padding: '6px 8px', borderRadius: 6, border: '1px solid #e2e8f0' }}>\n        {actual.desc}\n      </div>\n    </div>\n  );\n}",
      options: ["42", "'42'", "true", "null"],
      answer: "42"
    }
  }, {
    ...
    question: {
      ...
      previewCode: "function VariableKeywordsPreview() {\n  const [seleccionado, setSeleccionado] = React.useState('let');\n  const items = [\n    { kw: 'let', valido: true, ejemplo: 'let x = 10;', desc: '\u2713 V\xE1lido: Declara una variable reasignable' },\n    { kw: 'const', valido: true, ejemplo: 'const y = 20;', desc: '\u2713 V\xE1lido: Declara una constante de bloque' },\n    { kw: 'style', valido: false, ejemplo: 'style z = 30;', desc: '\u2717 Inv\xE1lido: \"style\" es una propiedad de estilos, no una palabra clave de declaraci\xF3n' },\n    { kw: 'return', valido: false, ejemplo: 'return w = 40;', desc: '\u2717 Inv\xE1lido: \"return\" devuelve valores desde funciones, no declara variables' }\n  ];\n  const actual = items.find(i => i.kw === seleccionado) || items[0];\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>Validador de Palabras Clave</span>\n        <span style={{ background: actual.valido ? '#10b981' : '#ef4444', color: 'white', fontWeight: 800, padding: '2px 8px', borderRadius: 6, fontSize: 12 }}>{actual.valido ? '\u2713 Palabra V\xE1lida' : '\u2717 Inv\xE1lida'}</span>\n      </div>\n      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(68px, 1fr))', gap: 6, marginBottom: 8 }}>\n        {items.map(it => (\n          <button key={it.kw} onClick={() => setSeleccionado(it.kw)} style={{ minHeight: 34, padding: '4px 8px', borderRadius: 6, border: '1.5px solid', borderColor: seleccionado === it.kw ? (it.valido ? '#10b981' : '#ef4444') : '#cbd5e1', background: seleccionado === it.kw ? (it.valido ? '#dcfce7' : '#fee2e2') : 'white', color: seleccionado === it.kw ? (it.valido ? '#15803d' : '#b91c1c') : '#334155', fontWeight: 700, fontSize: 12, cursor: 'pointer' }}>{it.kw}</button>\n        ))}\n      </div>\n      <div style={{ padding: '6px 8px', background: 'white', borderRadius: 6, border: '1px solid #e2e8f0', fontFamily: 'monospace', fontSize: 11.5, color: '#0f172a', marginBottom: 4 }}>\n        {actual.ejemplo}\n      </div>\n      <div style={{ fontSize: 10.5, color: actual.valido ? '#15803d' : '#b91c1c', fontWeight: 600 }}>\n        {actual.desc}\n      </div>\n    </div>\n  );\n}",
      options: ["let", "const", "style", "return"],
      answer: ["let", "const"]
    }
  }, {
    ...
    question: {
      ...
      previewCode: "function VariableAccumulatorPreview() {\n  const [paso, setPaso] = React.useState(0);\n  const pasos = [\n    { etiqueta: 'Paso 1: let count = 1;', val: 1, desc: 'Variable count creada con valor inicial 1' },\n    { etiqueta: 'Paso 2: count += 2;', val: 3, desc: 'Suma 2 al valor anterior (1 + 2 = 3)' },\n    { etiqueta: 'Paso 3: console.log(count);', val: 3, desc: 'Imprime el valor acumulado 3 en consola' }\n  ];\n  const actual = pasos[paso];\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 12, fontWeight: 700, color: '#334155' }}>Estado de Variable en Vivo</span>\n        <span style={{ background: '#3b82f6', color: 'white', fontWeight: 800, padding: '3px 8px', borderRadius: 6, fontSize: 13 }}>count = {actual.val}</span>\n      </div>\n      <div style={{ padding: '8px 10px', background: 'white', borderRadius: 6, border: '1px solid #e2e8f0', marginBottom: 8 }}>\n        <div style={{ fontSize: 12, fontWeight: 700, color: '#0f172a', fontFamily: 'monospace' }}>{actual.etiqueta}</div>\n        <div style={{ fontSize: 11, color: '#64748b', marginTop: 2 }}>{actual.desc}</div>\n      </div>\n      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>\n        <button onClick={() => setPaso(s => Math.min(pasos.length - 1, s + 1))} disabled={paso === pasos.length - 1} style={{ flex: '1 1 120px', minHeight: 34, padding: '4px 8px', borderRadius: 6, background: paso === pasos.length - 1 ? '#cbd5e1' : '#ec4899', color: 'white', border: 0, fontWeight: 700, fontSize: 11.5, cursor: paso === pasos.length - 1 ? 'default' : 'pointer' }}>Siguiente L\xEDnea \u2192</button>\n        <button onClick={() => setPaso(0)} style={{ minHeight: 34, padding: '4px 10px', borderRadius: 6, background: '#f1f5f9', color: '#475569', border: '1px solid #cbd5e1', fontWeight: 600, fontSize: 11.5, cursor: 'pointer' }}>Reiniciar</button>\n      </div>\n    </div>\n  );\n}",
      code: "let count = 1;\ncount += 2;\nconsole.log(count);",
      options: ["1", "2", "3", "undefined"],
      answer: "3"
    }
    ...
    question: {
      ...
      previewCode: "function ConstLetMutationPreview() {\n  const [useLet, setUseLet] = React.useState(false);\n  const [score, setScore] = React.useState(1);\n  const [error, setError] = React.useState(false);\n  const handleIncrement = () => {\n    if (!useLet) {\n      setError(true);\n    } else {\n      setScore(s => s + 1);\n      setError(false);\n    }\n  };\n  const handleToggle = (toLet) => {\n    setUseLet(toLet);\n    setError(false);\n    setScore(1);\n  };\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>Palabra de declaraci\xF3n:</span>\n        <div style={{ display: 'flex', gap: 4 }}>\n          <button onClick={() => handleToggle(false)} style={{ padding: '3px 8px', borderRadius: 5, border: '1px solid', borderColor: !useLet ? '#ef4444' : '#cbd5e1', background: !useLet ? '#fee2e2' : 'white', color: !useLet ? '#b91c1c' : '#64748b', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>const (Error)</button>\n          <button onClick={() => handleToggle(true)} style={{ padding: '3px 8px', borderRadius: 5, border: '1px solid', borderColor: useLet ? '#10b981' : '#cbd5e1', background: useLet ? '#dcfce7' : 'white', color: useLet ? '#15803d' : '#64748b', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>let (Soluci\xF3n \u2713)</button>\n        </div>\n      </div>\n      <div style={{ padding: '8px 10px', background: 'white', borderRadius: 6, border: '1px solid #e2e8f0', marginBottom: 8, fontFamily: 'monospace', fontSize: 11.5 }}>\n        <div>{useLet ? 'let' : 'const'} score = 1;</div>\n        <div style={{ color: '#64748b' }}>score += 1;</div>\n      </div>\n      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 6 }}>\n        <button onClick={handleIncrement} style={{ minHeight: 34, padding: '4px 12px', borderRadius: 6, background: '#3b82f6', color: 'white', border: 0, fontWeight: 700, fontSize: 11.5, cursor: 'pointer' }}>Ejecutar score += 1</button>\n        {error ? (\n          <span style={{ background: '#fee2e2', color: '#b91c1c', padding: '3px 8px', borderRadius: 6, fontSize: 11, fontWeight: 700 }}>\u2717 TypeError: Asignaci\xF3n a constante</span>\n        ) : (\n          <span style={{ background: '#dcfce7', color: '#15803d', padding: '3px 8px', borderRadius: 6, fontSize: 11.5, fontWeight: 800 }}>\u2713 score = {score}</span>\n        )}\n      </div>\n    </div>\n  );\n}",
      starterCode: "const score = 1;\nscore += 1;",
      answer: "let score = 1;\nscore += 1;",
    ...
    question: {
      ...
      previewCode: "function ArrowFunctionPreview() {\n  const [a, setA] = React.useState(3);\n  const [b, setB] = React.useState(5);\n  const resultado = a + b;\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>Funci\xF3n Flecha: const sumar = (a, b) =&gt; a + b;</span>\n        <span style={{ background: '#3b82f6', color: 'white', fontWeight: 800, padding: '2px 8px', borderRadius: 6, fontSize: 12.5 }}>sumar({a}, {b}) = {resultado}</span>\n      </div>\n      <div style={{ display: 'flex', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>\n        <div style={{ flex: '1 1 100px' }}>\n          <label style={{ fontSize: 10.5, color: '#64748b', fontWeight: 600, display: 'block', marginBottom: 2 }}>Par\xE1metro a: {a}</label>\n          <input type=\"range\" min=\"1\" max=\"20\" value={a} onChange={(e) => setA(Number(e.target.value))} style={{ width: '100%' }} />\n        </div>\n        <div style={{ flex: '1 1 100px' }}>\n          <label style={{ fontSize: 10.5, color: '#64748b', fontWeight: 600, display: 'block', marginBottom: 2 }}>Par\xE1metro b: {b}</label>\n          <input type=\"range\" min=\"1\" max=\"20\" value={b} onChange={(e) => setB(Number(e.target.value))} style={{ width: '100%' }} />\n        </div>\n      </div>\n      <div style={{ padding: '6px 8px', background: '#0f172a', borderRadius: 6, color: '#38bdf8', fontSize: 11, fontFamily: 'monospace', overflowX: 'auto', whiteSpace: 'nowrap' }}>\n        <span style={{ color: '#f472b6' }}>const</span> sumar = ({a}, {b}) =&gt; {a} + {b} <span style={{ color: '#10b981' }}>// retorna {resultado}</span>\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function GradeBranchPreview() {\n  const [puntaje, setPuntaje] = React.useState(85);\n  const calificacion = puntaje >= 90 ? 'A' : puntaje >= 80 ? 'B' : 'C';\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 12, fontWeight: 700, color: '#334155' }}>Puntaje: {puntaje}</span>\n        <span style={{ background: calificacion === 'A' ? '#10b981' : calificacion === 'B' ? '#3b82f6' : '#f59e0b', color: 'white', fontWeight: 800, padding: '2px 8px', borderRadius: 6, fontSize: 13 }}>Nota: {calificacion}</span>\n      </div>\n      <input type=\"range\" min=\"50\" max=\"100\" value={puntaje} onChange={(e) => setPuntaje(Number(e.target.value))} style={{ width: '100%', marginBottom: 10 }} />\n      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 11, fontFamily: 'monospace' }}>\n        <div style={{ padding: '4px 6px', borderRadius: 4, background: puntaje >= 90 ? '#dcfce7' : '#f1f5f9', color: puntaje >= 90 ? '#15803d' : '#94a3b8', borderLeft: puntaje >= 90 ? '3px solid #22c55e' : '3px solid transparent', overflowX: 'auto', whiteSpace: 'nowrap' }}>if (puntaje &gt;= 90) return \"A\" {puntaje >= 90 ? '\u2713 Activo' : ''}</div>\n        <div style={{ padding: '4px 6px', borderRadius: 4, background: puntaje < 90 && puntaje >= 80 ? '#dbeafe' : '#f1f5f9', color: puntaje < 90 && puntaje >= 80 ? '#1d4ed8' : '#94a3b8', borderLeft: puntaje < 90 && puntaje >= 80 ? '3px solid #3b82f6' : '3px solid transparent', overflowX: 'auto', whiteSpace: 'nowrap' }}>else if (puntaje &gt;= 80) return \"B\" {puntaje < 90 && puntaje >= 80 ? '\u2713 Activo' : ''}</div>\n        <div style={{ padding: '4px 6px', borderRadius: 4, background: puntaje < 80 ? '#fef3c7' : '#f1f5f9', color: puntaje < 80 ? '#b45309' : '#94a3b8', borderLeft: puntaje < 80 ? '3px solid #f59e0b' : '3px solid transparent', overflowX: 'auto', whiteSpace: 'nowrap' }}>else return \"C\" {puntaje < 80 ? '\u2713 Activo' : ''}</div>\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function AgeBoundaryPreview() {\n  const [edad, setEdad] = React.useState(18);\n  const resultadoBug = edad > 18;\n  const resultadoFix = edad >= 18;\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 12, fontWeight: 700, color: '#334155' }}>Edad Ingresada: {edad}</span>\n        <span style={{ fontSize: 11, color: '#64748b' }}>Mayor de edad: 18+</span>\n      </div>\n      <input type=\"range\" min=\"15\" max=\"21\" value={edad} onChange={(e) => setEdad(Number(e.target.value))} style={{ width: '100%', marginBottom: 10 }} />\n      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>\n        <div style={{ flex: '1 1 120px', padding: '6px 8px', background: resultadoBug ? '#dcfce7' : '#fee2e2', borderRadius: 6, border: '1px solid', borderColor: resultadoBug ? '#86efac' : '#fca5a5' }}>\n          <div style={{ fontSize: 10, color: '#64748b', fontWeight: 700 }}>Error: if (edad &gt; 18)</div>\n          <div style={{ fontSize: 12, fontWeight: 800, color: resultadoBug ? '#15803d' : '#b91c1c' }}>{resultadoBug ? '\u2713 Permitido' : '\u2717 Denegado'}</div>\n        </div>\n        <div style={{ flex: '1 1 120px', padding: '6px 8px', background: resultadoFix ? '#dcfce7' : '#fee2e2', borderRadius: 6, border: '1px solid', borderColor: resultadoFix ? '#86efac' : '#fca5a5' }}>\n          <div style={{ fontSize: 10, color: '#64748b', fontWeight: 700 }}>Corregido: if (edad &gt;= 18)</div>\n          <div style={{ fontSize: 12, fontWeight: 800, color: resultadoFix ? '#15803d' : '#b91c1c' }}>{resultadoFix ? '\u2713 Permitido' : '\u2717 Denegado'}</div>\n        </div>\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function TruthyFalsyPreview() {\n  const [selected, setSelected] = React.useState('0');\n  const values = [\n    { label: '0', val: 0 },\n    { label: '\"\"', val: '' },\n    { label: 'null', val: null },\n    { label: 'undefined', val: undefined },\n    { label: 'NaN', val: NaN },\n    { label: 'false', val: false },\n    { label: '\"0\"', val: '0' },\n    { label: '[]', val: [] },\n    { label: '{}', val: {} },\n  ];\n  const activeItem = values.find(v => v.label === selected);\n  const isTruthy = Boolean(activeItem ? activeItem.val : false);\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 10, border: '1px solid #cbd5e1' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 13, fontWeight: 700, color: '#334155' }}>Boolean({selected}) se evalúa como:</span>\n        <span style={{ background: isTruthy ? '#10b981' : '#ef4444', color: 'white', fontWeight: 800, padding: '4px 10px', borderRadius: 6, fontSize: 13 }}>{isTruthy ? '\u2713 Truthy (Verdadero)' : '\u2717 Falsy (Falso)'}</span>\n      </div>\n      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(64px, 1fr))', gap: 6 }}>\n        {values.map(v => (\n          <button key={v.label} onClick={() => setSelected(v.label)} style={{ minHeight: 38, padding: '6px 8px', borderRadius: 7, border: '1.5px solid', borderColor: selected === v.label ? '#2563eb' : '#cbd5e1', background: selected === v.label ? '#dbeafe' : 'white', color: selected === v.label ? '#1e40af' : '#1e293b', fontWeight: 700, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{v.label}</button>\n        ))}\n      </div>\n    </div>\n  );\n}",
      options: ["0", '""', "null", "undefined", "NaN", "false", '"0"', "[]", "{}"],
      answer: ["0", '""', "null", "undefined", "NaN", "false"]
    }
  }, {
    ...
    description: "Completa la condición de la cláusula de guarda para detener entradas inválidas.",
    isFillCodeBlanks: true,
    showPreview: true,
    question: {
      questionText: "Completa la cláusula de guarda para retornar temprano si el monto es menor o igual a cero:",
      previewCode: "function GuardClausePreview() {\n  const [monto, setMonto] = React.useState(50);\n  const esInvalido = monto <= 0;\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 12, fontWeight: 700, color: '#334155' }}>Pago: ${monto}</span>\n        <span style={{ background: esInvalido ? '#ef4444' : '#10b981', color: 'white', fontWeight: 800, padding: '2px 8px', borderRadius: 6, fontSize: 12 }}>{esInvalido ? 'Retorno Temprano: Inv\xE1lido' : 'Aprobado: $' + monto}</span>\n      </div>\n      <input type=\"range\" min=\"-20\" max=\"100\" value={monto} onChange={(e) => setMonto(Number(e.target.value))} style={{ width: '100%', marginBottom: 10 }} />\n      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 11, fontFamily: 'monospace' }}>\n        <div style={{ padding: '4px 6px', borderRadius: 4, background: esInvalido ? '#fee2e2' : '#f1f5f9', color: esInvalido ? '#b91c1c' : '#94a3b8', borderLeft: esInvalido ? '3px solid #ef4444' : '3px solid transparent', overflowX: 'auto', whiteSpace: 'nowrap' }}>if (monto &lt;= 0) return 'Monto inv\xE1lido' {esInvalido ? '\u2190 Se detiene aqu\xED' : ''}</div>\n        <div style={{ padding: '4px 6px', borderRadius: 4, background: !esInvalido ? '#dcfce7' : '#f1f5f9', color: !esInvalido ? '#15803d' : '#94a3b8', borderLeft: !esInvalido ? '3px solid #22c55e' : '3px solid transparent', overflowX: 'auto', whiteSpace: 'nowrap' }}>return 'Aprobado' {!esInvalido ? '\u2190 Procesado' : '(Omitido)'}</div>\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function LoopTracePreview() {\n  const [pasoIndice, setPasoIndice] = React.useState(0);\n  const estados = [\n    { etiqueta: 'Inicio', i: null, prev: null, desc: 'Estado inicial antes del bucle', resultado: 1, esFinal: false },\n    { etiqueta: 'i = 0', i: 0, prev: 1, desc: '1 \xD7 2', resultado: 2, esFinal: false },\n    { etiqueta: 'i = 1', i: 1, prev: 2, desc: '2 \xD7 2', resultado: 4, esFinal: false },\n    { etiqueta: 'i = 2', i: 2, prev: 4, desc: '4 \xD7 2', resultado: 8, esFinal: true },\n  ];\n  const actual = estados[pasoIndice];\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 10, border: '1px solid #cbd5e1' }}>\n      <div style={{ display: 'flex', gap: 6, marginBottom: 10 }}>\n        {estados.map((s, idx) => (\n          <button key={s.etiqueta} onClick={() => setPasoIndice(idx)} style={{ flex: 1, padding: '6px 4px', borderRadius: 6, border: '1px solid', borderColor: pasoIndice === idx ? '#3b82f6' : idx < pasoIndice ? '#86efac' : '#cbd5e1', background: pasoIndice === idx ? '#eff6ff' : idx < pasoIndice ? '#f0fdf4' : 'white', color: pasoIndice === idx ? '#1d4ed8' : idx < pasoIndice ? '#166534' : '#64748b', fontWeight: 700, fontSize: 11.5, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3 }}>\n            {idx < pasoIndice && '\u2713 '}{s.etiqueta}\n          </button>\n        ))}\n      </div>\n      <div style={{ padding: '10px 12px', background: 'white', borderRadius: 8, border: '1px solid #e2e8f0', marginBottom: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>\n        <div>\n          <div style={{ fontSize: 11, color: '#64748b', fontWeight: 600, marginBottom: 2 }}>\n            {actual.i === null ? 'Declaraci\xF3n: let conteo = 1;' : `Iteraci\xF3n ${actual.etiqueta} (conteo *= 2):`}\n          </div>\n          <div style={{ fontSize: 14, fontWeight: 800, color: '#0f172a', fontFamily: 'monospace' }}>\n            {actual.prev !== null ? `${actual.prev} \xD7 2 = ${actual.resultado}` : 'conteo = 1'}\n          </div>\n        </div>\n        <div style={{ textAlign: 'right' }}>\n          <span style={{ fontSize: 10, color: '#64748b', fontWeight: 700, display: 'block', textTransform: 'uppercase' }}>Valor</span>\n          <span style={{ background: actual.esFinal ? '#10b981' : '#3b82f6', color: 'white', fontWeight: 800, padding: '3px 9px', borderRadius: 6, fontSize: 13 }}>\n            conteo = {actual.resultado}\n          </span>\n        </div>\n      </div>\n      <div style={{ display: 'flex', gap: 6 }}>\n        <button onClick={() => setPasoIndice(s => Math.min(estados.length - 1, s + 1))} disabled={pasoIndice === estados.length - 1} style={{ flex: 1, minHeight: 38, padding: '6px 12px', borderRadius: 7, background: pasoIndice === estados.length - 1 ? '#cbd5e1' : '#3b82f6', color: 'white', border: 0, fontWeight: 700, fontSize: 12.5, cursor: pasoIndice === estados.length - 1 ? 'default' : 'pointer' }}>\n          {pasoIndice === estados.length - 1 ? 'Bucle Completado (console.log \u2192 8)' : 'Siguiente Iteraci\xF3n \u2192'}\n        </button>\n        <button onClick={() => setPasoIndice(0)} style={{ minHeight: 38, padding: '6px 14px', borderRadius: 7, background: '#f1f5f9', color: '#475569', border: '1px solid #cbd5e1', fontWeight: 700, fontSize: 12.5, cursor: 'pointer' }}>\n          Reiniciar\n        </button>\n      </div>\n    </div>\n  );\n}",
      code: "let conteo = 1;\nfor (let i = 0; i < 3; i++) {\n  conteo *= 2;\n}\nconsole.log(conteo);",
      options: ["8", "6", "4", "16"],
      answer: "8"
    }
    ...
    question: {
      ...
      previewCode: "function TemplateLiteralPreview() {\n  const [nombre, setNombre] = React.useState('Alex');\n  const [rol, setRol] = React.useState('Desarrollador');\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>\n        <input value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder=\"Nombre\" style={{ flex: '1 1 90px', minHeight: 32, padding: '4px 8px', fontSize: 11.5, borderRadius: 5, border: '1px solid #cbd5e1', boxSizing: 'border-box' }} />\n        <select value={rol} onChange={(e) => setRol(e.target.value)} style={{ flex: '1 1 120px', minHeight: 32, padding: '4px 6px', fontSize: 11.5, borderRadius: 5, border: '1px solid #cbd5e1', background: 'white' }}>\n          <option value=\"Desarrollador\">Desarrollador</option>\n          <option value=\"Dise\xF1ador\">Dise\xF1ador</option>\n          <option value=\"Gerente\">Gerente</option>\n        </select>\n      </div>\n      <div style={{ padding: '8px 10px', background: '#0f172a', borderRadius: 6, color: '#f8fafc', fontSize: 11.5, fontFamily: 'monospace', overflowX: 'auto', whiteSpace: 'nowrap' }}>\n        <span style={{ color: '#38bdf8' }}>`Usuario </span>\n        <span style={{ background: '#ec4899', color: 'white', padding: '1px 4px', borderRadius: 3 }}>${'{nombre}'}</span>\n        <span style={{ color: '#38bdf8' }}> es un </span>\n        <span style={{ background: '#8b5cf6', color: 'white', padding: '1px 4px', borderRadius: 3 }}>${'{rol}'}</span>\n        <span style={{ color: '#38bdf8' }}>.`</span>\n      </div>\n      <div style={{ marginTop: 6, fontSize: 11.5, color: '#0f172a', fontWeight: 600 }}>\n        Resultado: <span style={{ color: '#16a34a' }}>\"Usuario {nombre} es un {rol}.\"</span>\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function ArrayMutationPreview() {\n  const [frutas, setFrutas] = React.useState(['manzana', 'platano', 'cereza']);\n  const [log, setLog] = React.useState('Arreglo inicial con 3 frutas.');\n  const handlePush = () => {\n    if (!frutas.includes('datil')) {\n      setFrutas([...frutas, 'datil']);\n      setLog('frutas.push(\"datil\") agreg\xF3 al final.');\n    }\n  };\n  const handleShift = () => {\n    if (frutas.length > 0) {\n      const eliminado = frutas[0];\n      setFrutas(frutas.slice(1));\n      setLog(`frutas.shift() elimin\xF3 \\\"${eliminado}\\\" del \xEDndice 0.`);\n    }\n  };\n  const handleReset = () => {\n    setFrutas(['manzana', 'platano', 'cereza']);\n    setLog('Arreglo restablecido al estado inicial.');\n  };\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ fontSize: 11, color: '#64748b', marginBottom: 6, wordBreak: 'break-word' }}>{log}</div>\n      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8, minHeight: 32, alignItems: 'center' }}>\n        {frutas.map((f, idx) => (\n          <span key={f + idx} style={{ padding: '3px 8px', borderRadius: 6, background: '#10b981', color: 'white', fontWeight: 700, fontSize: 11 }}>[{idx}] {f}</span>\n        ))}\n        {frutas.length === 0 && <span style={{ color: '#94a3b8', fontSize: 11 }}>Arreglo vac\xEDo []</span>}\n      </div>\n      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>\n        <button onClick={handlePush} style={{ flex: '1 1 80px', minHeight: 34, padding: '4px 6px', borderRadius: 5, background: '#3b82f6', color: 'white', border: 0, fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>.push('datil')</button>\n        <button onClick={handleShift} style={{ flex: '1 1 80px', minHeight: 34, padding: '4px 6px', borderRadius: 5, background: '#f59e0b', color: 'white', border: 0, fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>.shift()</button>\n        <button onClick={handleReset} style={{ minHeight: 34, padding: '4px 10px', borderRadius: 5, background: '#f1f5f9', color: '#475569', border: '1px solid #cbd5e1', fontWeight: 600, fontSize: 11, cursor: 'pointer' }}>Reiniciar</button>\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function ObjectDestructuringPreview() {\n  const producto = { titulo: 'Auriculares', precio: 99 };\n  const [desestructurado, setDesestructurado] = React.useState(true);\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>Objeto Origen: producto</span>\n        <button onClick={() => setDesestructurado(d => !d)} style={{ padding: '3px 8px', minHeight: 30, borderRadius: 5, border: '1px solid #3b82f6', background: '#eff6ff', color: '#1d4ed8', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>{desestructurado ? 'Mostrar Notaci\xF3n Punto' : 'Mostrar Desestructuraci\xF3n'}</button>\n      </div>\n      <div style={{ padding: '8px 10px', background: 'white', borderRadius: 6, border: '1px solid #e2e8f0', marginBottom: 8 }}>\n        <div style={{ fontSize: 11, color: '#64748b', marginBottom: 4 }}>{desestructurado ? 'const { titulo, precio } = producto;' : 'const titulo = producto.titulo; const precio = producto.precio;' }</div>\n        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>\n          <span style={{ padding: '3px 8px', borderRadius: 6, background: '#dbeafe', color: '#1d4ed8', fontWeight: 700, fontSize: 11 }}>titulo = \"{producto.titulo}\"</span>\n          <span style={{ padding: '3px 8px', borderRadius: 6, background: '#dcfce7', color: '#15803d', fontWeight: 700, fontSize: 11 }}>precio = ${producto.precio}</span>\n        </div>\n      </div>\n      <div style={{ fontSize: 11.5, color: '#0f172a', fontWeight: 600 }}>\n        Salida Formateada: <strong style={{ color: '#ec4899' }}>{producto.titulo}: ${producto.precio}</strong>\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function ObjectSpreadPreview() {\n  const [tema, setTema] = React.useState('oscuro');\n  const original = { usuario: 'Maya', tema: 'claro', volumen: 80 };\n  const actualizado = { ...original, tema };\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>Propagaci\xF3n Inmutable (Spread)</span>\n        <button onClick={() => setTema(t => t === 'oscuro' ? 'claro' : 'oscuro')} style={{ padding: '3px 8px', minHeight: 30, borderRadius: 5, border: '1px solid #8b5cf6', background: '#f5f3ff', color: '#6d28d9', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>Alternar tema: {tema}</button>\n      </div>\n      <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>\n        <div style={{ flex: '1 1 120px', padding: '6px 8px', background: 'white', borderRadius: 6, border: '1px solid #cbd5e1', boxSizing: 'border-box' }}>\n          <div style={{ fontSize: 10, color: '#64748b', fontWeight: 700 }}>original (Sin Modificar)</div>\n          <div style={{ fontSize: 11, fontFamily: 'monospace', color: '#334155' }}>tema: \"{original.tema}\"</div>\n        </div>\n        <div style={{ flex: '1 1 120px', padding: '6px 8px', background: '#f0fdf4', borderRadius: 6, border: '1px solid #86efac', boxSizing: 'border-box' }}>\n          <div style={{ fontSize: 10, color: '#15803d', fontWeight: 700 }}>actualizado = {'{ ...original, tema }'}</div>\n          <div style={{ fontSize: 11, fontFamily: 'monospace', color: '#166534', fontWeight: 700 }}>tema: \"{actualizado.tema}\"</div>\n        </div>\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function ClassInstancePreview() {\n  const [instancias, setInstancias] = React.useState([\n    { nombre: 'Maya', correo: 'maya@dev.io' }\n  ]);\n  const [nombre, setNombre] = React.useState('');\n  const handleSpawn = () => {\n    if (nombre.trim()) {\n      setInstancias([...instancias, { nombre: nombre.trim(), correo: `${nombre.toLowerCase().replace(/\\s+/g, '')}@dev.io` }]);\n      setNombre('');\n    }\n  };\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>\n        <input value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder=\"Nombre de usuario (ej. Sam)\" style={{ flex: '1 1 120px', minHeight: 34, padding: '4px 8px', fontSize: 11.5, borderRadius: 6, border: '1px solid #cbd5e1', boxSizing: 'border-box' }} />\n        <button onClick={handleSpawn} style={{ minHeight: 34, padding: '4px 12px', background: '#ec4899', color: 'white', border: 0, borderRadius: 6, fontWeight: 700, fontSize: 11.5, cursor: 'pointer' }}>new Usuario()</button>\n      </div>\n      <div style={{ fontSize: 11, color: '#64748b', marginBottom: 4, fontWeight: 700 }}>Instancias activas de clase ({instancias.length}):</div>\n      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>\n        {instancias.map((inst, i) => (\n          <div key={i} style={{ padding: '4px 8px', background: 'white', border: '1px solid #3b82f6', borderRadius: 6, fontSize: 11, wordBreak: 'break-all' }}>\n            <strong style={{ color: '#1d4ed8' }}>{inst.nombre}</strong> <span style={{ color: '#64748b', fontSize: 10 }}>({inst.correo})</span>\n          </div>\n        ))}\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function DetachedContextPreview() {\n  const [saldo, setSaldo] = React.useState(100);\n  const [error, setError] = React.useState(null);\n  const handleMethodCall = () => {\n    setSaldo(s => s + 50);\n    setError(null);\n  };\n  const handleDetachedCall = () => {\n    setError('TypeError: Cannot read properties of undefined (reading saldo)');\n  };\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>cuenta = new CuentaBancaria(100)</span>\n        <span style={{ background: '#10b981', color: 'white', fontWeight: 800, padding: '2px 8px', borderRadius: 6, fontSize: 12 }}>saldo: ${saldo}</span>\n      </div>\n      <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>\n        <button onClick={handleMethodCall} style={{ flex: '1 1 120px', minHeight: 34, padding: '4px 6px', borderRadius: 5, background: '#3b82f6', color: 'white', border: 0, fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>cuenta.depositar(50) (this \u2713)</button>\n        <button onClick={handleDetachedCall} style={{ flex: '1 1 120px', minHeight: 34, padding: '4px 6px', borderRadius: 5, background: '#ef4444', color: 'white', border: 0, fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>desacoplado(50) (this \u2717)</button>\n      </div>\n      {error && (\n        <div style={{ padding: '6px 8px', background: '#fee2e2', borderRadius: 6, color: '#b91c1c', fontSize: 11, fontFamily: 'monospace', overflowX: 'auto', whiteSpace: 'nowrap' }}>\n          \u2717 {error}\n        </div>\n      )}\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function ArrayTransformPreview() {\n  const [usuarios, setUsuarios] = React.useState([\n    { id: 1, nombre: 'Ana', estaActivo: true },\n    { id: 2, nombre: 'Beto', estaActivo: false },\n    { id: 3, nombre: 'Carlos', estaActivo: true },\n  ]);\n  const nombresActivos = usuarios.filter(u => u.estaActivo).map(u => u.nombre);\n  const toggleUser = (id) => {\n    setUsuarios(usuarios.map(u => u.id === id ? { ...u, estaActivo: !u.estaActivo } : u));\n  };\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ fontSize: 11, fontWeight: 700, color: '#64748b', marginBottom: 6 }}>Alternar estado de usuario:</div>\n      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>\n        {usuarios.map(u => (\n          <button key={u.id} onClick={() => toggleUser(u.id)} style={{ flex: '1 1 80px', minHeight: 34, padding: '5px 8px', borderRadius: 6, border: '1px solid', borderColor: u.estaActivo ? '#10b981' : '#cbd5e1', background: u.estaActivo ? '#dcfce7' : 'white', color: u.estaActivo ? '#15803d' : '#64748b', fontSize: 11.5, fontWeight: 700, cursor: 'pointer', textAlign: 'center' }}>{u.nombre}: {u.estaActivo ? 'Activo \u2713' : 'Inactivo'}</button>\n        ))}\n      </div>\n      <div style={{ padding: '6px 8px', background: '#0f172a', borderRadius: 6, color: '#f8fafc', fontSize: 11, fontFamily: 'monospace', overflowX: 'auto', whiteSpace: 'nowrap' }}>\n        <span style={{ color: '#38bdf8' }}>nombresActivos</span> = [{nombresActivos.map(n => `\"${n}\"`).join(', ')}]\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function ObjectReferencePreview() {\n  const [puntaje, setPuntaje] = React.useState(25);\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>Objeto Compartido en Memoria</span>\n        <span style={{ background: '#8b5cf6', color: 'white', fontWeight: 800, padding: '2px 8px', borderRadius: 6, fontSize: 12 }}>puntaje = {puntaje}</span>\n      </div>\n      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>\n        <div style={{ flex: '1 1 120px', padding: '6px 8px', background: 'white', borderRadius: 6, border: '1px solid #3b82f6', boxSizing: 'border-box' }}>\n          <div style={{ fontSize: 10, color: '#1d4ed8', fontWeight: 700 }}>usuarioA \u2192 (Puntero)</div>\n          <div style={{ fontSize: 12, fontWeight: 800, color: '#0f172a' }}>puntaje: {puntaje}</div>\n        </div>\n        <div style={{ flex: '1 1 120px', padding: '6px 8px', background: 'white', borderRadius: 6, border: '1px solid #ec4899', boxSizing: 'border-box' }}>\n          <div style={{ fontSize: 10, color: '#be185d', fontWeight: 700 }}>usuarioB = usuarioA \u2192</div>\n          <div style={{ fontSize: 12, fontWeight: 800, color: '#0f172a' }}>puntaje: {puntaje}</div>\n        </div>\n      </div>\n      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>\n        <span style={{ fontSize: 10.5, color: '#64748b', fontWeight: 600 }}>Mutar usuarioB.puntaje:</span>\n        <input type=\"range\" min=\"10\" max=\"50\" value={puntaje} onChange={(e) => setPuntaje(Number(e.target.value))} style={{ flex: '1 1 100px', minWidth: 100 }} />\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function OptionalChainingPreview() {\n  const [tieneDireccion, setTieneDireccion] = React.useState(false);\n  const usuario = tieneDireccion ? { perfil: { direccion: { ciudad: 'Tokio' } } } : { perfil: null };\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>Estado de usuario.perfil:</span>\n        <button onClick={() => setTieneDireccion(a => !a)} style={{ padding: '4px 8px', minHeight: 30, borderRadius: 5, border: '1px solid #3b82f6', background: '#eff6ff', color: '#1d4ed8', fontWeight: 700, fontSize: 11, cursor: 'pointer' }}>Alternar {tieneDireccion ? 'Dirección: Tokio' : 'perfil: null'}</button>\n      </div>\n      <div style={{ display: 'flex', flexDirection: 'column', gap: 4, fontSize: 11, fontFamily: 'monospace' }}>\n        <div style={{ padding: '4px 6px', borderRadius: 4, background: tieneDireccion ? '#dcfce7' : '#fee2e2', color: tieneDireccion ? '#15803d' : '#b91c1c', overflowX: 'auto', whiteSpace: 'nowrap' }}>usuario.perfil.direccion.ciudad \u2192 {tieneDireccion ? '\"Tokio\"' : '\u2717 TypeError: No se puede leer propiedad' }</div>\n        <div style={{ padding: '4px 6px', borderRadius: 4, background: '#dcfce7', color: '#15803d', overflowX: 'auto', whiteSpace: 'nowrap' }}>usuario?.perfil?.direccion?.ciudad \u2192 {tieneDireccion ? '\"Tokio\"' : 'undefined (\u2713 Seguro)'}</div>\n      </div>\n    </div>\n  );\n}",
    ...
    question: {
      ...
      previewCode: "function ClassMethodStatePreview() {\n  const [count, setCount] = React.useState(5);\n  const [historial, setHistorial] = React.useState(['new Contador(5) \u2192 count = 5']);\n  const handleIncrement = (en) => {\n    setCount(prev => {\n      const next = prev + en;\n      setHistorial(h => [...h.slice(-2), `increment(${en}) \u2192 count = ${next}`]);\n      return next;\n    });\n  };\n  return (\n    <div style={{ fontFamily: 'system-ui, sans-serif', padding: '10px 12px', background: '#f8fafc', borderRadius: 8, border: '1px solid #cbd5e1', maxWidth: '100%', boxSizing: 'border-box' }}>\n      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>\n        <span style={{ fontSize: 11.5, fontWeight: 700, color: '#334155' }}>c = new Contador(5)</span>\n        <span style={{ background: '#ec4899', color: 'white', fontWeight: 800, padding: '2px 8px', borderRadius: 6, fontSize: 12.5 }}>c.count = {count}</span>\n      </div>\n      <div style={{ display: 'flex', gap: 6, marginBottom: 8, flexWrap: 'wrap' }}>\n        <button onClick={() => handleIncrement(3)} style={{ flex: '1 1 100px', minHeight: 34, padding: '4px 6px', borderRadius: 5, background: '#3b82f6', color: 'white', border: 0, fontWeight: 700, fontSize: 11.5, cursor: 'pointer' }}>c.increment(3)</button>\n        <button onClick={() => handleIncrement(1)} style={{ flex: '1 1 100px', minHeight: 34, padding: '4px 6px', borderRadius: 5, background: '#10b981', color: 'white', border: 0, fontWeight: 700, fontSize: 11.5, cursor: 'pointer' }}>c.increment(1)</button>\n      </div>\n      <div style={{ padding: '6px 8px', background: '#0f172a', borderRadius: 6, color: '#38bdf8', fontSize: 11, fontFamily: 'monospace' }}>\n        {historial.map((linea, idx) => (\n          <div key={idx}>{linea}</div>\n        ))}\n      </div>\n    </div>\n  );\n}",
