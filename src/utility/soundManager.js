...
class SoundManager {
  ...
  play(name) {
    // Auto-init on first play attempt if allowed
  ...
  sounds = {
    ...
    learn: () => {
        ...
        const synth = this.createDisposablePolySynth(
            { type: "sine", attack: 0.05, decay: 0.5, sustain: 0.1, release: 0.5 },
            VOL.NORMAL,
            1000
        );
        const now = Tone.now();
        synth.triggerAttackRelease(["C4", "E4", "G4", "B4", "C5"], "16n", now);
    },

    // ADDED: Flashcard sound
    flashcard: () => {
