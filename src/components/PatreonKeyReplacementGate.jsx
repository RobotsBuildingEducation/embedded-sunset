  ...
  const content = (
    <VStack align="stretch" spacing={{ base: 5, md: 6 }}>
      <Box textAlign="center">
        ...
        <Text color="purple.400" fontSize="xs" fontWeight="black" letterSpacing="wide" textTransform="uppercase" mb={2}>{copy.eyebrow}</Text>
      ...
      <VStack spacing={3}>
        <Button w="100%" h="auto" py={5} bg="purple.400" color="white" boxShadow="0 4px 0 #6b46c1" onClick={onConfirm} isLoading={isChecking} loadingText={copy.replacing} _hover={{ bg: "purple.500", color: "white", transform: "translateY(-1px)" }} _active={{ bg: "purple.600", color: "white", transform: "translateY(2px)" }}>{copy.confirm}</Button>
        <Button w="100%" variant="ghost" onClick={onCancel} isDisabled={isChecking}>{copy.cancel}</Button>
      </VStack>
